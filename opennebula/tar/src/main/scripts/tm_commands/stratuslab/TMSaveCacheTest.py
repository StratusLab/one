...
from tm_save_cache import TMSaveCache

class TMSaveCacheTest(TestCase):
    ...
    def setUp(self):
        self.tm = TMSaveCache([], config='stratuslab.cfg')
    ...
    def test_should_return_instance_id_when_in_path(self):
        self.tm.diskSrcPath = 'host-42:/the/path/with/inst/42/stuff/foo.0.bar'
        self.tm._getInstanceId()
        self.assertEqual(self.tm.instanceId, 42)

    def test_should_raise_when_missing_instance_id_in_path(self):
        self.tm.diskSrcPath = 'host-42:/the/path/with/inst/wrong-id1-42/stuff/foo.0.bar'
    ...
    def test_should_raise_when_too_many_candidates_in_path_for_instance_id(self):
        self.tm.diskSrcPath = 'host-42:/the/path/with/inst/42/43/stuff/foo.0.bar'
        self.assertRaises(ValueError, self.tm._getInstanceId)
